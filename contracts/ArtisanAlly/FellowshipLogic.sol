...
import {LSP7DigitalAssetInitAbstract} from "../UpgradeSafeLSPs/lsp7-contracts/contracts/LSP7DigitalAssetInitAbstract.sol";
...
contract FellowshipLogic is Initializable, LSP7DigitalAssetInitAbstract {
    ...
    function setup(
        ...
        emit SettedUp(backerbuckInitialPrice, backerbuckPriceGrowth);
    }
    ...
    function mint(uint256 amount) public payable {
        if (_backerbuckInitialPrice == 0) {
            revert NotInitializedYet();
        }
    ...
    event SubIntialization(
        address contributionTokenAddress,
        address endorsementTokenAddress
    );
